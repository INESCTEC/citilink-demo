...
def extract_text_from_doc(input_path: str, output_path: str, entidade: str) -> bool:
    """
    Extract text from a document (DOC, DOCX, or PDF) and save it to a text file.
    
    Args:
        input_path (str): Path to the input document
        output_path (str): Path where the text file will be saved
        entidade (str): Entity identifier (e.g., "CM_COVILHA", "AM_PORTO")
        
    Returns:
        bool: True if successful, False otherwise
    """
    try:
        print(f"Starting text extraction for {input_path}")
        print(f"Entity: {entidade}")
        
        if input_path.endswith(".pdf"):
            print("Converting PDF to DOCX")
            input_path = docx_of_pdf(input_path)
        elif input_path.endswith(".doc"):
            print("Converting DOC to DOCX")
            input_path = docx_of_doc(input_path)
        
        print(f"Processing file: {input_path}")
        
        if entidade.startswith("CM"):
            if "COVILHA" in entidade:
                print("Using CM_Covilha processor")
                CM_Covilha(input_path, output_path)
            elif "ALANDROAL" in entidade:
                print("Using docx_to_txt_pandoc with header extraction")
                docx_to_txt_pandoc(input_path, output_path, _extract_header=True)
            elif "CAMPOMAIOR" in entidade:
                print("Using CM_campo_maior processor")
                CM_campo_maior(input_path, output_path)
            elif "FUNDAO" in entidade:
                print("Using CM_alandroal processor")
                CM_alandroal(input_path, output_path)
            elif "PORTO" in entidade:
                print("Using docx_to_txt_pandoc")
                docx_to_txt_pandoc(input_path, output_path)
            elif "GUIMARAES" in entidade:
                print("Using CM_alandroal processor with Guimarães entity")
                CM_alandroal(input_path, output_path, 
                            entidade="Câmara Municipal de Guimarães")
            else:
                print("No specific processor found, using default")
                docx_to_txt_pandoc(input_path, output_path)
        ...
        if os.path.exists(output_path):
            with open(output_path, 'r', encoding='utf-8') as f:
                content = f.read()
                print(f"Output file created successfully. Content length: {len(content)}")
                print(f"First 200 characters: {content[:200]}")
                
                # Check if content is empty
                if not content.strip():
                    print("Error: Extracted content is empty")
                    return f"error: No text content could be extracted from the document"
...
# Only run the command-line interface if the script is run directly
if __name__ == "__main__":
    import argparse
    
    parser = argparse.ArgumentParser(prog='python txt_of_doc.py')
    ...
    success = extract_text_from_doc(args.input, args.output, args.entidade)
